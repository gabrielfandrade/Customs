...
function s.filter(c)
	return c:IsAbleToHand() and c:IsAttribute(ATTRIBUTE_WATER) and (c:IsFaceup() or c:IsLocation(LOCATION_GRAVE))
end
...
function s.spfilter(c,e,tp)
	return c:IsSetCard(0x6a4) and c:IsLevelBelow(5) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
...
function s.thfilter(c)
	return c:IsSetCard(0x6a4) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand()
end
