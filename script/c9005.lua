...
function s.spfilter(c,ft)
	return c:IsFaceup() and c:IsSetCard(0x3dc) and not c:IsCode(id) and c:IsAbleToHandAsCost()
...
function s.spfilter1(c,ft)
	return c:IsFaceup() and c:IsSetCard(0x3dc) and c:IsAbleToGraveAsCost() and (ft>0 or c:GetSequence()<5)
end
function s.spcon1(e,c)
	if c==nil then return true end
	local tp=c:GetControler()
	local ft=Duel.GetLocationCount(tp,LOCATION_MZONE)
	local rg=Duel.GetMatchingGroup(s.spfilter1,tp,LOCATION_MZONE,0,nil,ft)
	return ft>-1 and #rg>0 and aux.SelectUnselectGroup(rg,e,tp,1,1,nil,0)
end
