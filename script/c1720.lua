...
function s.spfilter(c,e,tp)
	return c:IsCode(1200912) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
...
function s.atkcon(e,tp,eg,ep,ev,re,r,rp)
	...
	e:SetLabelObject(c)
	return c and c~=e:GetHandler() and c:IsAttribute(ATTRIBUTE_LIGHT) and c:IsRelateToBattle() and e:GetHandler():GetAttackAnnouncedCount()<1
end
...
function s.cfilter(c)
	return c:IsCode(1200912) and (c:IsLocation(LOCATION_HAND+LOCATION_DECK) or c:IsFaceup()) and c:IsAbleToGraveAsCost()
end
...
function s.synfilter(c,e,tp)
	return c:IsCode(1200911) and Duel.GetLocationCountFromEx(tp,tp,nil,c)>0 and c:IsCanBeSpecialSummoned(e,0,tp,false,true)
end
