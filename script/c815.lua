...
function s.rfilter(c)
	return c:IsAttribute(ATTRIBUTE_WIND) and c:IsAttackAbove(3000)
end
...
function s.thtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsOnField() and s.sthfilter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(s.sthfilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil)
	...
	local tg=Duel.SelectTarget(tp,s.sthfilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,ct,nil)
	Duel.SetOperationInfo(0,CATEGORY_TODECK,tg,ct,0,0)
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,tg,1-tp,0)
end
...
function s.spfilter(c,e,tp)
	return c:IsAttribute(ATTRIBUTE_WIND) and c:GetLevel()==10 and not c:IsCode(id) and c:IsCanBeSpecialSummoned(e,0,tp,true,true)
end
