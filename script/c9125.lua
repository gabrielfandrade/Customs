--Served HERO Knight of the Diamond Dragon of North
local s,id=GetID()
...
function s.afilter(c,fc,sumtype,tp,sub,mg,sg,contact)
	...
	return c:IsAttribute(ATTRIBUTE_WATER,fc,sumtype,tp) and (not contact or c:IsType(TYPE_MONSTER,fc,sumtype,tp))
end
...
function s.subval(e,c)
	return c:IsSetCard(0x3e7) and c:IsAttribute(ATTRIBUTE_WATER)
end
function s.filter(c)
